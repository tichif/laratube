        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                  ...
                  <a href="{{ route('channel.upload', $channel->id) }}" class="btn btn-primary">Upload videos</a>
                </div>
                ...
                <div class="card-body">
                    ...
                    @if ($channel->editable())
                      <input onchange="document.getElementById('update-channel-form').submit()" type="file" style="display:none" name="image" id="image">


                      <div class="form-group">
                        <label for="name" class="form-control-label">Name</label>
                        <input type="text" name="name" id="name" value="{{ $channel->name }}" class="form-control" placeholder="Enter your name">
                      </div>
                      <div class="form-group">
                        <label for="description" class="form-control-label">Description</label>
                        <textarea name="description" id="description" class="form-control" placeholder="Enter the description">{{ $channel->description }}</textarea>
                      </div>
                  ...
                  <tbody>
                    @foreach ($videos as $i => $video)
                        <tr>
                          <td>{{ ++$i}}</td>
                          <td>
                            <img width="40px" height="40px" src="{{ $video->thumbnail }}" alt="{{ $video->title }}">
                          </td>
                          <td>{{ $video->title }}</td>
                          <td>{{ $video->views }}</td>
                          <td>{{ $video->percentage == 100 ? 'Completed' :'Processing'}}</td>
                          <td>
                            @if ( $video->percentage == 100)
                              <a href="{{ url('videos') }}/{{ $video->id }}" target="_blank" class="btn btn-info btn-sm">View</a>
                            @endif
                          </td>
                        </tr>
                    @endforeach
